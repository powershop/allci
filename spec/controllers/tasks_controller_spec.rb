...
RSpec.describe TasksController, type: :controller do
  describe "#pull" do
    it "receives queue pull requests and passes the data to AssignTask, returning the task in JSON format" do
      ...
      expect(AssignTask).to receive(:new).with(build_id: "containers", stage: "build_component_images", runner_id: "foo-12:1").and_return(service)
      expect(service).to receive(:call).and_return(build_task)
      expect(build_task).to receive(:to_json).and_return("{ foo: 'bar' }")

      post :pull, params: { task: { id: "containers", stage: "build_component_images", runner_id: "foo-12:1" } }
